...
class OleFileIO:
    ...
    def _open(self, start, size=UNKNOWN_SIZE, force_FAT=False):
        ...
        if size < self.minisectorcutoff and not force_FAT:
            ...
            return OleStream(fp=self.fp, sect=start, size=size,
                             ...
                             olefileio=self)
    def _list2(self, files, prefix, node, streams=True, storages=False):
        """
        listdir helper
    ...
    def listdir2(self, streams=True, storages=False):
        """
        ...
        """
        files = []
        self._list2(files, [], self.root, streams, storages)
        return files
    ...
    def _list(self, files, prefix, node, streams=True, storages=False):
        ...
        :param storages: bool, include storages if True (False by default) - new in v0.26
         ...
         : param files : 채울 파일 목록
         : param prefix : 스토리지 트리의 현재 위치 (이름 목록)
         : param node : 현재 노드 (OleDirectoryEntry 객체)
         : param streams : bool, True 인 경우 스트림 포함 (기본값은 True)-v0.26의 새로운 기능
         : param storages : bool, true 인 경우 스토리지 포함 (기본값은 False)-v0.26의 새로운 기능
             (참고 : 루트 저장소는 포함되지 않습니다.)
        """
        prefix = prefix + [node.name]
        for entry in node.kids:
            if entry.entry_type == STGTY_STORAGE:
                ...
                # this is a stream
                binloc = node.name == "BinData"
                if binloc:
                    # add it to the list
                    files.append(prefix[1:] + [entry.name])
                docloc = node.name == "DocOptions"
                if docloc:
                    files.append(prefix[1:] + [entry.name])
                scploc = node.name == "Scripts"
                if scploc:
                    files.append(prefix[1:] + [entry.name])
                secloc = node.name == "BodyText"
                if secloc:
                    if entry.name != "Section0":
                        files.append(prefix[1:] + [entry.name])
            else:
                self._raise_defect(DEFECT_INCORRECT,
                                   'The directory tree contains an entry which is not a stream nor a storage.')
    ...
    def listStream(self, number, streams=True, storages=False):
        files = []
        self._listCDR(number, files, [], self.root, streams, storages)

        return files

    def _listCDR(self, number, files, prefix, node, streams=True, storages=False):
        prefix = prefix + [node.name]
        for entry in node.kids:
            if entry.entry_type == STGTY_STORAGE:
                # this is a storage
                if storages:
                    ...
                    files.append(prefix[1:] + [entry.name])
                # check its kids
                self._listCDR(number, files, prefix, entry, streams, storages)
            elif entry.entry_type == STGTY_STREAM:
                # this is a stream
                if number == 3:
                    if 'DocInfo' not in entry.name:
                        if '\x05HwpSummaryInformation' not in entry.name:
                            if 'PrvText' not in entry.name:
                                if 'PrvImage' not in entry.name:
                                    if 'FileHeader' not in entry.name:
                                        if 'BodyText' not in prefix[1:]:
                                            if 'ViewText' not in prefix[1:]:
                                                files.append(prefix[1:] + [entry.name])
                    ...
                    if entry.name not in '\x05HwpSummaryInformation' 'PrvText' 'PrvImage' 'DocInfo' 'FileHeader':
                        if jpg not in entry.name:
                            if bmp not in entry.name:
                                if rle not in entry.name:
                                    if dib not in entry.name:
                                        if gif not in entry.name:
                                            if png not in entry.name:
                                                if tif not in entry.name:
                                                    if tiff not in entry.name:
                                                        if raw not in entry.name:
                                                            if jpeg not in entry.name:
                                                                if img not in entry.name:
                                                                    if 'BodyText' not in prefix[1:]:
                                                                        if 'ViewText' not in prefix[1:]:
                                                                            files.append(prefix[1:] + [entry.name])
                                                        ...
                                                        if raw not in entry.name:
                                                            if jpeg not in entry.name:
                                                                if img not in entry.name:
                                                                    if 'BodyText' not in prefix[1:]:
                                                                        if 'ViewText' not in prefix[1:]:
                                                                            if 'DocHistory' not in prefix[1:]:
                                                                                if 'XML Template' not in prefix[1:]:
                                                                                    if 'Scripts' not in prefix[1:]:
                                                                                        if 'DocOptions' not in prefix[1:]:
                                                                                            files.append(prefix[1:] + [entry.name])
